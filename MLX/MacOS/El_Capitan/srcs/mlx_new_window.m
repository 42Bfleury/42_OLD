...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/11/11 00:08:35 by bfleury           #+#    #+#             */
/*   Updated: 2016/11/11 00:08:38 by bfleury          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*mlx_new_window(mlx_t *mlx, int width, int height, char *title)
{
...
void	mlx_clear_window(mlx_t *mlx, mlx_win_list_t *win)
{
	[(id)(win->winid) selectGLContext];
	[(id)(win->winid) clearWin];
	win->nb_flush ++;
}
...
void	mlx_hook(mlx_win_list_t *win, int x_event, int x_mask, int (*f)(), void *p)
{
	[(id)(win->winid) setEvent:x_event andFunc:f andParam:p];
}

void	mlx_expose_hook(mlx_win_list_t *win, int (*f)(), void *p)
{
	[(id)(win->winid) setEvent:12 andFunc:f andParam:p];
}

void	mlx_mouse_hook(mlx_win_list_t *win, int (*f)(), void *p)
{
	[(id)(win->winid) setEvent:4 andFunc:f andParam:p];
}

void	mlx_key_hook(mlx_win_list_t *win, int (*f)(), void *p)
{
	[(id)(win->winid) setEvent:3 andFunc:f andParam:p];
}
